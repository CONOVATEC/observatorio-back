<div class="card">
    <div class="card-header mt-1 mb-0 py-0">
        <h4 class="card-title">Listado de políticas eliminadas</h4>
                    ...
                    <div class="table-responsive ">
                        <table class="table table-striped " style="width:100%">
                            ...
                            <tbody>
                                ...
                                @forelse($policiesEliminated as $polityEliminated)
                                <tr>
                                    {{-- <td>{{ $categoryEliminated->id }}</td> --}}
                                    <td>{{ $loop->iteration }}</td>
                                    <td><span class="d-inline-block text-truncate" style="max-width: 150px;">{{ $polityEliminated->name }}</span></td>
                                    <td class=""><span class="d-inline-block text-truncate" style="max-width: 250px;">{{ $polityEliminated->descripcion }}</span></td>
                                    <td><span class="badge rounded-pill badge-light-danger me-1">{{ $polityEliminated->deleted_at->format('d-m-Y') }}</span></td>
                                    <td class="text-center">
                                        {{-- Incluimos los botones  --}}
                                        @include('admin.pages.policy.partials.buttons-restore')
                                    </td>
                                </tr>
                                @empty
    ...
    <div class="card-footer">
        ...
        @include('admin.pages.policy.partials.pagination-restore')
    </div>
