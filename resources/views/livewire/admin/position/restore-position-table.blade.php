<div class="card">
    <div class="card-header mt-1 mb-0 py-0">
        <h4 class="card-title">Listado de puestos eliminados</h4>
                    ...
                    <div class="table-responsive ">
                        <table class="table table-striped " style="width:100%">
                            ...
                            <tbody>
                                ...
                                @forelse($positionsEliminated as $positionEliminated)
                                <tr>
                                    {{-- <td>{{ $categoryEliminated->id }}</td> --}}
                                    <td>{{ $loop->iteration }}</td>
                                    <td><span class="d-inline-block text-truncate" style="max-width: 150px;">{{ $positionEliminated->name }}</span></td>
                                    <td><span class="badge rounded-pill badge-light-danger me-1">{{ $positionEliminated->deleted_at->format('d-m-Y') }}</span></td>
                                    @if(auth()->user()->can('posiciones.eliminar.definitivo') or auth()->user()->can('posiciones.restaurar') )
                                    <td class="text-center">
                                        {{-- Incluimos los botones  --}}
                                        @include('admin.pages.position.partials.buttons-restore')
                                    </td>
                                    @endif
                                </tr>
                                @empty
    ...
    <div class="card-footer">
        ...
        @include('admin.pages.position.partials.pagination-restore')
    </div>
