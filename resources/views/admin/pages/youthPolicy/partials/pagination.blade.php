<div class="overflow-auto flex-sm-fill d-sm-flex align-items-sm-center justify-content-sm-between">
    ...
    <div>
        <p class="small text-muted">
            {!! __('Showing') !!}
            <span class="font-medium">{{ $youthPolicies->firstItem() }}</span>
            {!! __('to') !!}
            <span class="font-medium">{{ $youthPolicies->lastItem() }}</span>
            {!! __('of') !!}
            <span class="font-medium">{{ $youthPolicies->total() }}</span>
            {!! __('results') !!}
        </p>
    ...
    <div class="">
        ...
        {{$youthPolicies->withQueryString()->links()}}
    </div>
</div>
