...
class RestorePostTable extends Component
{
    ...
    public function render()
    {
        ...
        $postEliminated = Post::onlyTrashed()->where('title', 'like', "%{$this->search}%");

      
        //Verificamos si el campo no son nuloss
        if ($this->camp and $this->order) {
            //Ejecuta la sentencia y lo agrega al usuario + el orderby
            $postEliminated = $postEliminated->orderBy($this->camp, $this->order);
        } else {
            $this->camp = null;
            $this->order = null;
        }
        $postEliminated = $postEliminated->onlyTrashed()->paginate($this->perPage);
        // $categoriesEliminated = Category::paginate(5);
        
        return view('livewire.admin.post.restore-post-table', compact('postEliminated'));
    }
