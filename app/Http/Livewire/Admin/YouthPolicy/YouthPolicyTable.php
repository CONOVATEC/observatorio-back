...
namespace App\Http\Livewire\Admin\YouthPolicy;

use Livewire\Component;
use Livewire\WithPagination;
use App\Models\admin\YouthPolicy;

class YouthPolicyTable extends Component
{
    use WithPagination;
    ...
    {
        $youthPolicies = YouthPolicy::where('name', 'like', "%{$this->search}%")
            ->orWhere('description', 'like', "%{$this->search}%")
            ->orWhere('link_drive', 'like', "%{$this->search}%")
            ->orWhere('link_video', 'like', "%{$this->search}%");

        //Verificamos si el campo no son nuloss
        if ($this->camp and $this->order) {
            //Ejecuta la sentensettings agrega al usuario + el orderby
            $youthPolicies = $youthPolicies->orderBy($this->camp, $this->order);
        } else {
            $this->camp = null;
            $this->order = null;
        }
        $youthPolicies = $youthPolicies->paginate($this->perPage);
        // $categories = Category::paginate(5);
        return view('livewire.admin.youth-policy.youth-policy-table', compact('youthPolicies'));
    }
