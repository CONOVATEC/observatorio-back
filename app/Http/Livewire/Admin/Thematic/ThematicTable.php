...
namespace App\Http\Livewire\Admin\Thematic;

use Livewire\Component;
use Livewire\WithPagination;
use App\Models\admin\Thematic;

class ThematicTable extends Component
{
    use WithPagination;
    ...
    {
        $thematics = Thematic::where('name', 'like', "%{$this->search}%")
        ...
        if ($this->camp and $this->order) {
            ...
            $thematics = $thematics->orderBy($this->camp, $this->order);
        } else {
            $this->camp = null;
            $this->order = null;
        }
        $thematics = $thematics->paginate($this->perPage);
        // $categories = Category::paginate(5);
        return view('livewire.admin.thematic.thematic-table', compact('thematics'));
