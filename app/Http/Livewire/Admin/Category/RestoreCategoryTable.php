...
class RestoreCategoryTable extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        $categoriesEliminated = Category::onlyTrashed()->where('name', 'like', "%{$this->search}%")
        ...
        if ($this->camp and $this->order) {
            ...
            $categoriesEliminated = $categoriesEliminated->orderBy($this->camp, $this->order);
        } else {
            $this->camp = null;
            $this->order = null;
        }
        $categoriesEliminated = $categoriesEliminated->onlyTrashed()->paginate($this->perPage);
        // $categoriesEliminated = Category::paginate(5);
        return view('livewire.admin.category.restore-category-table', compact('categoriesEliminated'));
    }
