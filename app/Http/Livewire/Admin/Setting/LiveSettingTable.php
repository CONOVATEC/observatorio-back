...
class LiveSettingTable extends Component
{
    use WithPagination;
    ...
    {
        $settings = Setting::where('name_entity', 'like', "%{$this->search}%")
        ...
        if ($this->camp and $this->order) {
            ...
            $settings = $settings->orderBy($this->camp, $this->order);
        } else {
            $this->camp = null;
            $this->order = null;
        }
        ...
        return view('livewire.admin.setting.live-setting-table', compact('settings'));
    }
