...
namespace App\Http\Livewire\Admin\TypeLogo;

use Livewire\Component;
use Livewire\WithPagination;
use App\Models\admin\TypeLogo;

class TypeLogoTable extends Component
{
    use WithPagination;
    ...
    {
        $typeLogos = TypeLogo::where('name', 'like', "%{$this->search}%")
        ...
        if ($this->camp and $this->order) {
            ...
            $typeLogos = $typeLogos->orderBy($this->camp, $this->order);
        } else {
            $this->camp = null;
            $this->order = null;
        }
        $typeLogos = $typeLogos->paginate($this->perPage);
        // $categories = Category::paginate(5);
        return view('livewire.admin.type-logo.type-logo-table', compact('typeLogos'));
