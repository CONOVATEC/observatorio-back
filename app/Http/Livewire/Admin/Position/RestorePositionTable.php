...
class RestorePositionTable extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        $positionsEliminated = Position::onlyTrashed()->where('name', 'like', "%{$this->search}%");
        //Verificamos si el campo no son nuloss
        if ($this->camp and $this->order) {
            //Ejecuta la sentencia y lo agrega al usuario + el orderby
            $positionsEliminated = $positionsEliminated->orderBy($this->camp, $this->order);
        } else {
            $this->camp = null;
            $this->order = null;
        }
        $positionsEliminated = $positionsEliminated->onlyTrashed()->paginate($this->perPage);
        // $categoriesEliminated = Category::paginate(5);
        return view('livewire.admin.position.restore-position-table', compact('positionsEliminated'));
    }
