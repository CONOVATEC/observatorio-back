...
class LivePositionTable extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        if ($this->camp and $this->order) {
            ...
            $positions = $positions->orderBy($this->camp, $this->order);
        } else {
            $this->camp = null;
            $this->order = null;
        }
        $positions = $positions->paginate($this->perPage);
        // $tags = Tag::paginate(5);
        return view('livewire.admin.position.live-position-table', compact('positions'));
    }
