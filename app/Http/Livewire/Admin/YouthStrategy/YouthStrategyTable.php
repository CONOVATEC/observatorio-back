...
namespace App\Http\Livewire\Admin\YouthStrategy;

use Livewire\Component;
use Livewire\WithPagination;
use App\Models\admin\YouthStrategy;

class YouthStrategyTable extends Component
{
    use WithPagination;
    ...
    {
        $youthStrategies = YouthStrategy::where('name', 'like', "%{$this->search}%");

        //Verificamos si el campo no son nuloss
        if ($this->camp and $this->order) {
            //Ejecuta la sentensettings agrega al usuario + el orderby
            $youthStrategies = $youthStrategies->orderBy($this->camp, $this->order);
        } else {
            $this->camp = null;
            $this->order = null;
        }
        $youthStrategies = $youthStrategies->paginate($this->perPage);
        // $categories = Category::paginate(5);
        return view('livewire.admin.youth-strategy.youth-strategy-table', compact('youthStrategies'));
    }
